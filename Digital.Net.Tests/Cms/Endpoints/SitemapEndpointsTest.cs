...
public class SitemapEndpointsTest
{
    ...
    public async Task GetSitemapData_ShouldReturnPublishedAndIndexedPages()
    {
        ...
        var notIndexed = context.BuildTestPage(published: true, indexed: false);
        var notPublished = context.BuildTestPage(published: false);
        ...
        await Assert.That(result!.Value!.Any(e => e.Path == notIndexed.Path)).IsFalse();
        await Assert.That(result!.Value!.Any(e => e.Path == notPublished.Path)).IsFalse();
    }
